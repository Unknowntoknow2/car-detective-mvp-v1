...
describe("generateValuationExplanation", () => {
    ...
    (calculateFinalValuation as any).mockReturnValue({
      adjustments: [
        ...
        },
        {
          ...
          impact: 650,
          description: "Premium features including Leather Seats, Sunroof",
          percentage: 3.25,
        },
      ],
      finalValue: 20250,
  ...
  it("should throw an error if the Supabase function returns an error", async () => {
    ...
    // Act & Assert
    await expect(generateValuationExplanation(params)).rejects.toThrow(
  ...
  it("should throw an error if no explanation is returned", async () => {
    ...
    // Act & Assert
    await expect(generateValuationExplanation(params)).rejects.toThrow(
      "No explanation received from server",
    );
  });
  ...
  it("should handle network errors gracefully", async () => {
    ...
    // Act & Assert
    await expect(generateValuationExplanation(params)).rejects.toThrow(
      "Failed to generate explanation: Network error",
    );
  });
});
