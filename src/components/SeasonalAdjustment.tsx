...
export function SeasonalAdjustment({
  ...
  const [bodyStyle, setBodyStyle] = useState<string | undefined>(
    defaultBodyStyle,
  );
  ...
  const fetchSeasonalTip = async (date: Date, bodyStyle: string) => {
    try {
      ...
      const { data, error } = await fetch(
        `/api/get-seasonal-factor?month=${month}&vehicleType=${vehicleType}`,
      )
        .then((res) => res.json());
      ...
      if (data) {
        ...
        setSeasonalTip(
          `Seasonal adjustment: ${percentChange}% (${direction} value)`,
        );
      }
    } catch (error) {
      console.error("Error fetching seasonal tip:", error);
  ...
  const handleRefreshSeasonalFactor = () => {
    if (!date || !bodyStyle) {
      toast.error(
        "Please select both a date and body style to calculate seasonal factors",
      );
      return;
    }
    ...
    const factor = factors[month as keyof typeof factors][
      vehicleType as keyof typeof factors[1]
    ];
    const percentChange = ((factor - 1) * 100).toFixed(1);
    const direction = factor > 1 ? "increases" : "decreases";
    ...
    toast.success(
      `Updated seasonal factors for ${format(date, "MMMM")} and ${bodyStyle}`,
    );
  };
