...
export function MarketTrends({ vehicleInfo }: MarketTrendsProps) {
  ...
  const trendDirection = priceDifference > 0
    ? "up"
    : priceDifference < 0
    ? "down"
    : "neutral";
        ...
        <div className="mt-4 pt-4 border-t grid grid-cols-3 gap-4">
          <div>
            ...
            <div className="font-medium">
              {formatCurrency(Math.min(...marketData.map((d) => d.value)))}
            </div>
          </div>
          <div>
            ...
            <div className="font-medium">
              {formatCurrency(
                marketData.reduce((sum, item) => sum + item.value, 0) /
                  marketData.length,
              )}
            </div>
          </div>
          <div>
            ...
            <div className="font-medium">
              {formatCurrency(Math.max(...marketData.map((d) => d.value)))}
            </div>
          </div>
        </div>
      </CardContent>
