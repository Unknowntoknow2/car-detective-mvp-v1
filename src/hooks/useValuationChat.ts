...
export function useValuationChat(valuationId?: string) {
  ...
  const fetchChatHistory = useCallback(async (sid: string) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch chat history"),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const createSession = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to create chat session"),
      );
      return null;
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err
          : new Error("Failed to initialize chat session"),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const sendMessage = useCallback(
    async (content: string, isRegeneration: boolean = false) => {
      if (!user) {
        toast.error("You must be logged in to use the chat feature");
        return;
      }
