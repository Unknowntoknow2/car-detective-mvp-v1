  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async (): Promise<DealerInsightsData> => {
      ...
      try {
        ...
        const acceptedOffers = offerData?.filter((offer) =>
          offer.status === "accepted"
        ).length || 0;
        const rejectedOffers = offerData?.filter((offer) =>
          offer.status === "rejected"
        ).length || 0;
        const pendingOffers = offerData?.filter((offer) =>
          ["sent", "viewed", "pending"].includes(offer.status)
        ).length || 0;

        // Calculate average score
        const scores = offerData?.map((offer) =>
          offer.score
        ).filter(Boolean) || [];
        const avgOfferScore = scores.length
        ...
        const offersByLabel = Object.entries(labelCounts).map((
          [label, count],
        ) => ({ label, count }));
        ...
        valuationsData?.forEach((valuation) => {
          const source = valuation.is_vin_lookup
            ? "VIN Lookup"
            : "Manual Entry";
