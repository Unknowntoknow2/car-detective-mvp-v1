...
export function useValidation<T>(schema: ZodSchema<T>) {
  const [errors, setErrors] = useState<Map<string, ValidationResult>>(
    new Map(),
  );
  ...
  const getFieldError = useCallback(
    (fieldName: string): ValidationResult | undefined => {
  ...
  switch (error.code) {
    case "too_small":
      return `Minimum length is ${
        (error as z.ZodIssue & { minimum?: number }).minimum
      } characters`;
    case "too_big":
      return `Maximum length is ${
        (error as z.ZodIssue & { maximum?: number }).maximum
      } characters`;
    case "invalid_string":
      return "Invalid format";
    case "invalid_type":
      return `Expected ${
        (error as z.ZodIssue & { expected?: string }).expected
      }, received ${(error as z.ZodIssue & { received?: string }).received}`;
    default:
      return undefined;
  }
}
