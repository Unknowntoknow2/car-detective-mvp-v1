...
const SYSTEM_PROMPT =
  `You are a professional auto inspector and valuation expert.
...
const USER_PROMPT =
  "Analyze these car photos and provide a condition assessment in the requested JSON format.";

// Call OpenAI API to analyze images
export async function analyzeImagesWithOpenAI(
  imageContents: string[],
): Promise<ConditionAssessmentResult | Response> {
  if (!OPENAI_API_KEY) {
  ...
  try {
    ...
    try {
      ...
      if (
        !assessmentResult.condition || !assessmentResult.confidenceScore ||
        !Array.isArray(assessmentResult.issuesDetected) ||
