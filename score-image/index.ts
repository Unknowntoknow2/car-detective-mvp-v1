...
serve(async (req) => {
  ...
  try {
    ...
    if (!photos || photos.length === 0 || !valuationId) {
      return new Response(
        JSON.stringify({
          error: "Missing required parameters: photos, valuationId",
        }),
        {
          status: 400,
    ...
    console.log(
      `Processing ${photos.length} photos for valuation ${valuationId}`,
    );
    ...
    const numIssues = selectedCondition === "Excellent"
      ? 0
    ...
    if (selectedCondition === "Excellent") {
      aiSummary =
        "Vehicle appears to be in excellent condition with minimal signs of wear.";
    } else if (selectedCondition === "Good") {
      aiSummary =
        "Vehicle is in good overall condition with minor cosmetic issues that don't affect functionality.";
    } else if (selectedCondition === "Fair") {
      aiSummary =
        "Vehicle shows signs of normal wear with some cosmetic issues that may require attention.";
    } else {
      aiSummary =
        "Vehicle has significant wear and may require repairs to restore to good condition.";
    }
    ...
    await supabase
      .from("valuations")
      .update({
        condition_score: selectedCondition === "Excellent"
          ? 90
          : selectedCondition === "Good"
          ? 75
          : selectedCondition === "Fair"
