...
describe("Row-Level Security (RLS) Access Controls", () => {
  ...
  beforeAll(async () => {
    // Sign in User A
    const { data: userAData, error: userAError } = await supabase.auth
      .signInWithPassword({
        email: USER_A.email,
    ...
    if (userAError) {
      throw new Error(`Failed to sign in User A: ${userAError.message}`);
    }
    userAToken = userAData.session?.access_token || "";

    // Sign in User B
    const { data: userBData, error: userBError } = await supabase.auth
      .signInWithPassword({
        email: USER_B.email,
    ...
    if (userBError) {
      throw new Error(`Failed to sign in User B: ${userBError.message}`);
    }
    ...
    if (userAValuationError) {
      throw new Error(
        `Failed to create valuation for User A: ${userAValuationError.message}`,
      );
    }
    ...
    if (userBValuationError) {
      throw new Error(
        `Failed to create valuation for User B: ${userBValuationError.message}`,
      );
    }
  ...
  it("should prevent downloading a premium report for an unpaid valuation", async () => {
    ...
    const attemptDownloadPremiumReport = async (
      valuationId: string,
    ...
    expect(result.error).toBe(
      "Premium features not unlocked for this valuation",
    );
  });
