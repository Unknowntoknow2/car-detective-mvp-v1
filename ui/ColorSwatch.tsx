...
export function ColorSwatch(
  { value, onChange, className, disabled = false }: ColorSwatchProps,
) {
  ...
  const colorsByCategory = colors.reduce<Record<string, ColorOption[]>>(
    (acc, color) => {
                ...
                <>
                  ...
                  {selectedColor && (
                    <span className="ml-2 text-xs text-muted-foreground">
                      ({selectedColor.multiplier > 1 ? "+" : ""}
                      {((selectedColor.multiplier - 1) * 100).toFixed(0)}%)
                    </span>
                  )}
                </>
              )
                        ...
                        >
                          <div className="flex items-center">
                            <div
                              ...
                              style={{
                                backgroundColor: getColorStyle(color.color),
                              }}
                            />
                            {color.color}
                          </div>
