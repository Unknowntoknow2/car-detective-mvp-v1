...
export function PriceDistributionChart({
  ...
  const priceStep = Math.round(
    (priceRange.highest - priceRange.lowest) / (distribution.length - 1),
  );
  ...
  const chartData = distribution.map((count, index) => {
    ...
    const label = `$${(minPrice / 1000).toFixed(0)}K - $${
      (maxPrice / 1000).toFixed(0)
    }K`;
    ...
    return {
      ...
      minPrice,
      maxPrice,
      isYourRange: priceRange.average >= minPrice &&
        priceRange.average <= maxPrice,
    };
  });
      ...
      <CardContent className="p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-sm text-slate-500">
            Based on {listingCount} similar {vehicleInfo.year}{" "}
            {vehicleInfo.make} {vehicleInfo.model} {vehicleInfo.trim || ""}{" "}
            listings
          </div>
        </div>
            ...
            >
              ...
              >
                {chartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.isYourRange
                      ? "#7c3aed"
                      : (highlightedBar === index ? "#a78bfa" : "#c4b5fd")}
                  />
                ))}
              </Bar>
            </BarChart>
        ...
        <div className="mt-6 grid grid-cols-3 text-center border-t border-slate-100 pt-4">
          <div>
            ...
            <p className="font-semibold">
              ${priceRange.lowest.toLocaleString()}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-semibold">
              ${priceRange.highest.toLocaleString()}
            </p>
          </div>
        </div>
          ...
          <div className="mt-4 space-y-4 p-4 bg-purple-50 rounded-lg text-sm">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-slate-600">
                  Your vehicle's estimated value is higher than approximately
                  {" "}
                  {valuePercentile}% of similar listings in the market.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-slate-600">
                  The most common price range is {mostCommonRange.range}, with
                  {" "}
                  {mostCommonRange.count} vehicles listed in this bracket.
                </p>
              </div>
            </div>
