  ...
  try {
    ...
    if (val.zip_code) {
      try {
        ...
        if (
          !marketAdjError && marketAdjData &&
          marketAdjData.market_multiplier !== null
        ) {
          multiplier = 1 + (marketAdjData.market_multiplier / 100);
          console.log(
            `Using market adjustment factor: ${multiplier} from market_adjustments table for zip: ${val.zip_code}`,
          );
        } else {
          ...
          const { data: marketData, error: marketErr } = await supabase
            .functions.invoke("fetch-market-listings", {
              body: {
          ...
          if (!marketErr && marketData && marketData.marketFactor) {
            ...
            if (!pricingErr && pricingData && pricingData.multiplier) {
              ...
              console.log(
                `Using pricing multiplier: ${multiplier} for zip: ${val.zip_code}, condition: ${condition}`,
              );
            }
          }
        }
    ...
    if (val.exterior_color) {
      try {
        ...
        if (val.color_multiplier) {
          ...
          console.log(
            `Using stored color multiplier: ${colorMultiplier} for color: ${val.exterior_color}`,
          );
        } else {
          ...
          if (!colorErr && colorData) {
            ...
            console.log(
              `Fetched color multiplier: ${colorMultiplier} for color: ${val.exterior_color}`,
            );
          }
        }
      } catch (err) {
    ...
    if (val.fuel_type) {
      try {
        ...
        if (!fuelTypeErr && fuelTypeData) {
          ...
          console.log(
            `Fetched fuel type multiplier: ${fuelTypeMultiplier} for fuel type: ${val.fuel_type}`,
          );
        }
      } catch (err) {
    ...
    if (val.transmission_type) {
      try {
        const { data: transmissionData, error: transmissionErr } =
          await supabase
        ...
        if (!transmissionErr && transmissionData) {
          ...
          console.log(
            `Fetched transmission multiplier: ${transmissionMultiplier} for transmission type: ${val.transmission_type}`,
          );
        }
      } catch (err) {
    ...
    if (val.has_open_recall) {
      try {
        ...
        if (!recallErr && recallData) {
          ...
          console.log(
            `Fetched recall multiplier: ${recallMultiplier} for vehicle with open recall`,
          );
        } else {
    ...
    if (val.warranty_status && val.warranty_status !== "None") {
      try {
        ...
        if (!warrantyErr && warrantyData) {
          ...
          console.log(
            `Fetched warranty multiplier: ${warrantyMultiplier} for warranty status: ${val.warranty_status}`,
          );
        } else {
          // Default based on warranty type
          warrantyMultiplier = val.warranty_status === "Factory" ? 1.02 : 1.04;
          console.log(
            `Using default warranty multiplier: ${warrantyMultiplier} for type: ${val.warranty_status}`,
          );
        }
      } catch (err) {
    ...
    if (val.driving_profile) {
      try {
        ...
        if (val.driving_profile_multiplier) {
          ...
          console.log(
            `Using stored driving profile multiplier: ${drivingProfileMultiplier} for profile: ${val.driving_profile}`,
          );
        } else {
          // Otherwise fetch from the driving_profile table
          const { data: drivingProfileData, error: drivingProfileErr } =
            await supabase
          ...
          if (!drivingProfileErr && drivingProfileData) {
            ...
            console.log(
              `Fetched driving profile multiplier: ${drivingProfileMultiplier} for profile: ${val.driving_profile}`,
            );
          }
        }
      } catch (err) {
    ...
    let intermediatePrice = basePredictedPrice * multiplier *
      (1 + photoAdjustment);
    ...
    if (val.sale_date && val.body_style) {
      try {
        ...
        if (bodyStyle.includes("suv") || bodyStyle.includes("crossover")) {
          ...
          bodyStyle.includes("convertible") || bodyStyle.includes("cabriolet")
        ) {
          vehicleType = "convertible";
        } else if (bodyStyle.includes("sport") || bodyStyle.includes("coupe")) {
          vehicleType = "sport";
        } else if (
          bodyStyle.includes("truck") || bodyStyle.includes("pickup")
        ) {
          vehicleType = "truck";
        }
        ...
        if (!seasonalErr && seasonalData) {
          ...
          console.log(
            `Fetched seasonal multiplier: ${seasonalMultiplier} for month: ${month}, vehicle type: ${vehicleType}`,
          );
      ...
      JSON.stringify({
        ...
        breakdown: {
          ...
          transmissionAdjustment: `${
            ((transmissionMultiplier - 1) * 100).toFixed(1)
          }%`,
          ...
          drivingProfileAdjustment: `${
            ((drivingProfileMultiplier - 1) * 100).toFixed(1)
          }%`,
