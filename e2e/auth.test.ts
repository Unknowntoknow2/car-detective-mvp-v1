...
test.describe("Authentication Flows", () => {
  test("allows a user to sign up with email and password", async ({ page }) => {
    ...
    await expect(page.getByText(/successfully|account created/i)).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("shows error message with incorrect login credentials", async ({ page }) => {
    ...
    await expect(page.getByText(/incorrect|invalid|wrong password|failed/i))
      .toBeVisible();
  });
  ...
  test("allows dealer signup with additional fields", async ({ page }) => {
    ...
    await expect(page.getByText(/successfully|account created|welcome/i))
  ...
  test("allows user to log out", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /log in|sign in|sign up/i }))
      .toBeVisible();
  });
});
