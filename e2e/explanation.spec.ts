...
test.describe("Explanation Feature", () => {
  ...
  test("should display AI confidence score in the explanation", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      globalThis.dispatchEvent(
        new CustomEvent("test:mockPremiumValuationSuccess", {
          detail: {
            make: "BMW",
            model: "3 Series",
            year: 2021,
    ...
    await expect(page.locator("text=This valuation has been AI-verified"))
      .toBeVisible();
  });
  ...
  test("should handle explanation generation failures gracefully", async ({ page }) => {
    ...
    await page.evaluate(() => {
      globalThis.dispatchEvent(new CustomEvent("test:mockExplanationFailure", {}));
    });
    ...
    await expect(page.locator('button:has-text("Regenerate Explanation")'))
      .toBeVisible();
    ...
    await page.evaluate(() => {
      globalThis.dispatchEvent(
        new CustomEvent("test:mockExplanationSuccess", {
          detail: {
            explanation:
              "This is a successfully regenerated explanation for your Honda Accord.",
          },
        }),
      );
    });
    ...
    await expect(
      page.locator("text=This is a successfully regenerated explanation"),
    ).toBeVisible();
  });
});
