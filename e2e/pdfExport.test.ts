...
test.describe("PDF Export Functionality", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto("/");
    await page.getByRole("link", { name: /valuation|estimate|appraisal/i })
      .click();
    await page.getByRole("button", { name: /manual entry|enter manually/i })
      .click();
    ...
    // Submit form
    await page.getByRole("button", { name: /submit|get valuation|next/i })
      .click();

    // Wait for results
    await expect(page.getByText(/valuation result|estimate|value/i))
  ...
  test("downloads PDF report from valuation result page", async ({ page }) => {
    ...
    page.on("download", async (download) => {
      const downloadPathFile = path.join(
        downloadPath,
    ...
    await page.getByRole("button", {
      name: /download pdf|export report|save as pdf/i,
    }).click();

    // Wait for download to start - look for success message
    await expect(
      page.getByText(/download started|generating pdf|download complete/i),
  ...
  test("premium report includes additional details", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /upgrade|get premium|unlock/i })
      .click();

    // If there's a test mode or bypass for premium, activate it
    // This will vary based on your implementation
    await page.getByRole("button", { name: /test mode|demo|skip payment/i })
      .click();
    ...
    await expect(page.getByText(/premium report|enhanced details|unlocked/i))
      .toBeVisible({ timeout: 20000 });

    // Download PDF
    await page.getByRole("button", {
      name: /download pdf|export premium report/i,
    }).click();
  ...
  test("shows error message if PDF generation fails", async ({ page }) => {
    ...
    await expect(page.getByText(/error|failed|could not generate/i))
      .toBeVisible();
  ...
  test("validates PDF is accessible from valuation history", async ({ page }) => {
    ...
    await expect(
      page.getByText(
        new RegExp(`${testVehicle.make}.*${testVehicle.model}`, "i"),
      ),
    ).toBeVisible();

    // Click view details or similar
    await page.getByRole("link", { name: /view|details|open/i }).first()
      .click();
