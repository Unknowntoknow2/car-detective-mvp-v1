...
test.describe("User Dashboard", () => {
  ...
  test("allows navigation between dashboard sections", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /profile|account/i }))
      .toBeVisible();
    await expect(page.getByRole("link", { name: /valuations|history/i }))
      .toBeVisible();
  ...
  test("updates user profile information", async ({ page }) => {
    ...
    await expect(page.getByText(/profile updated|changes saved|success/i))
      .toBeVisible();
  ...
  test("displays empty state when no valuations exist", async ({ page }) => {
    ...
    await expect(page.getByText(/no valuations|empty|start by getting/i))
      .toBeVisible();

    // Verify CTA to create valuation exists
    await expect(page.getByRole("link", { name: /get valuation|start now/i }))
      .toBeVisible();
  });
});
