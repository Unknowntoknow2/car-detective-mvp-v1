...
test.describe("Premium Payment Flow", () => {
  test("should display premium button and start checkout flow", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Unlock Premium Report/i }))
      .toBeVisible();
    ...
    await page.route("**/functions/v1/create-checkout", async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          url: "/valuation/premium-success?session_id=mock_session_123",
        }),
      });
    });
    ...
    await expect(page.getByRole("button", { name: /Download Premium Report/i }))
      .toBeVisible();
  });
  ...
  test("should handle payment cancellation", async ({ page }) => {
    ...
    await page.route("**/functions/v1/create-checkout", async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          url: "/valuation/premium?id=test-valuation-id&canceled=true",
        }),
      });
    });
