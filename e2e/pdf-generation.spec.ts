...
test.describe("PDF Generation", () => {
  ...
  test("Free valuation PDF downloads and contains base details", async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Download Report")', {
      timeout: 10000,
    });
  ...
  test("Premium PDF contains enhanced content and is accessible after payment", async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Download Premium Report")', {
      timeout: 10000,
    });

    // Download the premium report
    const downloadPromise = page.waitForEvent("download");
    await page.getByRole("button", { name: /download premium report/i })
      .click();
