...
test.describe("Valuation Flow", () => {
  ...
  test("offers premium upgrade option on valuation results", async ({ page }) => {
    ...
    await expect(page.getByText(/premium|upgrade|detailed report/i))
      .toBeVisible();
    await expect(
      page.getByRole("button", { name: /upgrade|get premium|unlock/i }),
    ).toBeVisible();
  });
  ...
  test("restricts premium features until payment", async ({ page }) => {
    ...
    await expect(page.getByText(/locked|premium only|upgrade to access/i))
      .toBeVisible();

    // Try to access a premium feature
    const premiumElement = page.getByText(
      /detailed history|forecast|dealer offers/i,
    ).first();
    await premiumElement.click();

    // Should show payment/upgrade prompt
    await expect(page.getByText(/payment|upgrade required|unlock now/i))
      .toBeVisible();
  });
  ...
  test("includes explanation for the valuation result", async ({ page }) => {
    ...
    await expect(page.getByText(/how we calculated|explanation|factors/i))
      .toBeVisible();

    // Check for specific valuation factors
    await expect(page.getByText(/mileage|condition|market|location/i))
      .toBeVisible();
  });
});
