...
test.describe("Valuation Flow", () => {
  ...
  test("should complete full valuation flow and premium purchase", async ({ page }) => {
    ...
    if (await premiumButton.isVisible()) {
      ...
      await expect(page.locator('button:has-text("Unlock Premium")'))
        .toBeVisible();
    }
    ...
    if (await offersSection.isVisible()) {
      ...
      const hasOffers = await page.locator('div:has-text("No offers yet")')
        .isVisible();
      const hasCta = await page.locator('button:has-text("Get Dealer Offers")')
        .isVisible();
    ...
    const adjustmentsSection = page.locator('h3:has-text("Value Adjustments")')
      .first();
  ...
  test("Plate lookup should validate state selection", async ({ page }) => {
    ...
    const isButtonDisabled = await page.locator('button:has-text("Lookup")')
      .isDisabled();
    const hasError = await page.locator("text=/select state|state required/i")
      .isVisible();
    ...
    const isLoading = await page.locator("text=/looking|searching/i")
      .isVisible();
