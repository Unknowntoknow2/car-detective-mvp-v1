...
test.describe("Free Valuation Flow", () => {
  test("should complete the free valuation form and display results", async ({ page }) => {
    ...
    // Select fuel type
    const fuelTypeSelect = page.locator('select, [role="combobox"]').filter({
      hasText: /Fuel Type/,
    });
    ...
    // Select condition
    const conditionSelect = page.locator('select, [role="combobox"]').filter({
      hasText: /Condition/,
    });
    ...
    const valuationCard = page.locator('h3:has-text("Estimated Value")')
      .first();
    ...
    const priceRange = page.locator(
      "text=/Estimated range: \\$[0-9,]+ - \\$[0-9,]+/",
    );
    await expect(priceRange).toBeVisible();

    // Verify "Get Premium Report" button/link is available
    const premiumButton = page.getByRole("button").filter({
      hasText: /Premium/i,
    });
  ...
  test("should show validation errors when required fields are missing", async ({ page }) => {
    ...
    await expect(page.locator("text=/required|invalid|missing/i"))
      .toBeVisible();
  ...
  test("should complete the VIN lookup flow and show results", async ({ page }) => {
    ...
    await page.getByPlaceholder(/Enter 17-character VIN/).fill(
      "4T1BF1FK5GU252261",
    );
