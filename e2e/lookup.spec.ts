...
test.describe("Vehicle Lookup Flows", () => {
  ...
  test("VIN lookup flow should work correctly", async ({ page }) => {
    ...
    await page.getByRole("link", { name: /VIN/i, exact: false }).first()
      .click();
  ...
  test("Manual entry flow should work correctly", async ({ page }) => {
    ...
    await expect(
      page.getByText(
        new RegExp(
          `${TEST_MANUAL_ENTRY.year}.*${TEST_MANUAL_ENTRY.make}.*${TEST_MANUAL_ENTRY.model}`,
          "i",
        ),
      ),
    ).toBeVisible();
  ...
  test("Premium valuation flow should work correctly", async ({ page }) => {
    ...
    await expect(page.getByText(/premium valuation/i, { exact: false }))
      .toBeVisible();

    // Check if premium features are visible
    await expect(page.getByText(/enhanced accuracy/i, { exact: false }))
      .toBeVisible();
    ...
    await expect(page.getByText(/carfax|vehicle history/i, { exact: false }))
      .toBeVisible();

    // Look for any premium-specific checkboxes and toggle them
    const historyCheckbox = page.getByRole("checkbox", {
      name: /include.*history|carfax/i,
    });
    ...
    await page.getByRole("button", { name: /get.*premium.*valuation|submit/i })
      .click();

    // Wait for the premium valuation result
    await page.waitForSelector("text=Premium Valuation Result", {
      timeout: 15000,
    });

    // Verify premium result elements are visible
    await expect(page.getByText(/confidence score/i, { exact: false }))
      .toBeVisible();
  });
