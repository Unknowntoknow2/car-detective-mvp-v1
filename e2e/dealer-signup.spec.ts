...
test.skip(
  !SUPABASE_KEY,
  "Skipping test because SUPABASE_SERVICE_ROLE_KEY is not set",
);
...
test.describe("Dealer Signup and Authentication Flow", () => {
  ...
  test("should allow dealer signup, login, and access to dealer dashboard", async ({ page }) => {
    ...
    await test.step("Navigate to dealer signup page", async () => {
      ...
      await expect(
        page.getByText(/Create Dealership Account|Register your dealership/),
      ).toBeVisible();
    });
    ...
    await test.step("Submit the dealer signup form", async () => {
      await page.getByRole("button", {
        name: /Create Dealer Account|Create Account|Register|Sign Up/i,
      }).click();
    ...
    await test.step("Handle email confirmation if needed", async () => {
      ...
      if (
        ...
        !currentUrl.includes("dealer-dashboard")
      ) {
        console.log(
          "Signup requires email confirmation - handling with direct login",
        );
        ...
        if (SUPABASE_KEY) {
          ...
          if (existingUsers && existingUsers.length > 0) {
            ...
            const { data: newUser, error } = await supabase.auth.admin
              .createUser({
          ...
          test.skip(
            true,
            "Email verification is required but we lack admin credentials to bypass it",
          );
        }
      } else {
    ...
    await test.step("Verify dealer dashboard content", async () => {
      ...
      await expect(page.getByText(`Dealership: ${testDealershipName}`))
        .toBeVisible();
      ...
      await expect(
        page.getByRole("button", { name: /Respond to Lead|View Valuations/i }),
      ).toBeVisible();
    });
  });
});
