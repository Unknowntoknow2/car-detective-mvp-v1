...
test.describe("VIN Lookup Functionality", () => {
  test("successfully looks up vehicle with valid VIN", async ({ page }) => {
    ...
    await expect(page.getByText(/vehicle details|results|make|model/i))
  ...
  test("shows appropriate error for invalid VIN", async ({ page }) => {
    ...
    await expect(page.getByText(/invalid vin|incorrect vin|not valid/i))
      .toBeVisible();
  });
  ...
  test("validates VIN length before submission", async ({ page }) => {
    ...
    await expect(page.getByText(/17 characters|valid vin|correct format/i))
      .toBeVisible();
  ...
  test("provides option to manually enter details from lookup page", async ({ page }) => {
    ...
    await expect(page.getByText(/vehicle details|make|model|year/i))
      .toBeVisible();
