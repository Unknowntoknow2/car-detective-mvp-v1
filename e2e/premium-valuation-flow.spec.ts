...
test.describe("Premium Valuation Flow", () => {
  test("should complete premium valuation form and display results", async ({ page }) => {
    ...
    const drivingProfiles = page.locator("button").filter({
      hasText: /Careful|Normal|Spirited/,
    });
    ...
    const downloadButton = page.getByRole("button").filter({
      hasText: /Download/,
    });
  ...
  test("should handle validation errors appropriately", async ({ page }) => {
    ...
    await expect(page.getByText(/required|invalid/, { exact: false }))
      .toBeVisible();
