...
test.describe("Dealer Inventory Management", () => {
  ...
  test("allows dealer to add a new vehicle to inventory", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /add vehicle|create listing/i })
      .click();
    ...
    await expect(page.getByText(/vehicle added|listing created|success/i))
      .toBeVisible();
  ...
  test("allows dealer to edit an existing vehicle", async ({ page }) => {
    ...
    // Verify success message
    await expect(page.getByText(/updated|changes saved|success/i))
      .toBeVisible();
  ...
  test("prevents non-dealer users from accessing inventory management", async ({ page }) => {
    ...
    const isAccessDenied = await Promise.race([
      page.getByText(/access denied|not authorized|dealer only/i).isVisible()
        .then((visible) => visible ? "denied" : ""),
      page.getByText(/sign in|log in|login required/i).isVisible().then(
