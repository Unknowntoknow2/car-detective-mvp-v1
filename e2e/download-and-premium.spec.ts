...
test.describe("Valuation and Premium Features", () => {
  ...
  test("should process AI photo upload and display condition tag", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      globalThis.dispatchEvent(
        new CustomEvent("test:mockPhotoUploadSuccess", {
          detail: {
            condition: "Good",
            confidenceScore: 85,
  ...
  test("should redirect to Stripe checkout for premium features", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      globalThis.sessionStorage.setItem("mockStripeSessionId", "mock_session_123");

      // Redirect to success page to simulate completion
      globalThis.location.href = "/premium-success?session_id=mock_session_123";
    });
