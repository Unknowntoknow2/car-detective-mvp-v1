...
import {
  initialValuationPipelineState,
  valuationPipelineReducer,
} from "./service";
import {
  PipelineActions,
  ValuationConditionData,
...
export function useValuationPipeline(): ValuationPipelineHook {
  ...
  const runLookup = useCallback(
    async (
      ...
      try {
        ...
        dispatch({
          type: "SET_STEP_COMPLETED",
          payload: { stepId: "vehicle-identification", isCompleted: true },
        });
  ...
  const submitValuation = useCallback(async (data: any) => {
    ...
    try {
      ...
      if (data.mileage || data.accidents || data.titleStatus) {
        dispatch({
          ...
          payload: {
            ...
            titleStatus: data.titleStatus || requiredInputs.titleStatus ||
              "Clean",
            ...
            mechanicalGrade: data.mechanicalGrade ||
              requiredInputs.mechanicalGrade,
      ...
      dispatch({
        type: "SET_STEP_COMPLETED",
        payload: { stepId: "vehicle-condition", isCompleted: true },
      });

      // Auto advance to result step when valuation is complete
      const resultStepIndex = state.steps.findIndex((step) =>
        step.id === "result"
      );
