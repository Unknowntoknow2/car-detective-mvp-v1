...
export async function fetchCarsDotComListings(
  ...
  const listings: any[] = [];
  const searchUrl =
    `https://www.cars.com/shopping/results/?stock_type=used&makes[]=${make.toLowerCase()}&models[]=${model.toLowerCase()}&list_price_max=&maximum_distance=100&zip=${zip}`;
  ...
  try {
    ...
    $("div.vehicle-card").each((_, el) => {
      ...
      const link = "https://www.cars.com" +
        $(el).find("a.vehicle-card-link").attr("href");
